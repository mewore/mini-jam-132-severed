...
project_name=''
lowercase_export_name=''

if [ -f "${PROJECT_FILE}" ]; then
  project_name=$(grep -oP '^config/name="\K[^"]+(?="$)' "${PROJECT_FILE}")
  echo "Project name: ${project_name}"
  read -rp "Export name [${project_name}]: " export_name
  if [ -z "${export_name}" ]; then export_name="${project_name}"; fi
  lowercase_export_name=$(echo "${export_name}" | tr '[:upper:]' '[:lower:]' | sed 's/[ :][ :]*/-/g')
  echo "Lowercase name: [${lowercase_export_name}]"
else
...
WINDOWS_SUB_DIR_NAME="${export_name}"
WINDOWS_EXECUTABLE="${export_name}.exe"
WINDOWS_DIR="${WINDOWS_BASE_DIR}/${WINDOWS_SUB_DIR_NAME}"

LINUX_BASE_DIR='linux64'
LINUX_SUB_DIR_NAME="${lowercase_export_name}"
LINUX_EXECUTABLE="${lowercase_export_name}.x86_64"
...
if [ "${is_test_web}" == 0 ]; then
  ...
  if wait "${windows_pid}"; then
    make_zip "${version_dir}/${WINDOWS_BASE_DIR}" "${version_dir}/${export_name} - Windows x64.zip" &
  else
    echo "Windows export failed: file://${windows_log}"
  fi

  if wait "${linux_pid}"; then
    make_tarball "${version_dir}/${LINUX_BASE_DIR}" "${version_dir}/${lowercase_export_name}-linux-x64.tar.gz" \
      "${LINUX_SUB_DIR_NAME}" &
  else
